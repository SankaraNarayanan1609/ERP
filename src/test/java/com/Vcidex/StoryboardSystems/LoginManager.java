...
public class LoginManager {
    ...
    /**
     * Performs login via API (useful for backend tests or RestAssured-based validations)
     *
     * Returns map of session details including:
     * - bearer token
     * - user ID
     * - company info
     *
     * @param appName     Application name
     * @param companyCode Company code (e.g., "vcidex")
     * @param userId      Username
     * @return Map with token, user_gid, company name, email, mobile, etc.
     */
    public Map<String, String> loginViaApiAndGetSession(String appName, String companyCode, String userId) {
        ...
        // Parse and clean bearer token
        String token = resp.jsonPath().getString("token");
        if (token != null && token.toLowerCase().startsWith("bearer ")) {
            token = token.substring(7); // remove "Bearer "
        }

        // Prepare return map with session context
